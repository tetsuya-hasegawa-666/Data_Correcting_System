...
- mRL-9-1 mobile-server connector status: `completed`
  - `Mobile --connector-- Server` を mobile / desktop 共通で表示する
- mRL-9-2 docker-down visibility: `completed`
  - docker 未起動時は `Server = ×` と赤 connector で見えるようにする
- mRL-9-3 compact settings rewrite: `completed`
  - settings を compact panel に圧縮し、clone 操作を主役に保つ
- mRL-9-4 docs and validation sync: `completed`
  - release lines、UX 手順、UX 自動検証レポート、現況を同期する
