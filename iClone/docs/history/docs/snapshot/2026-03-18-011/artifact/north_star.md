...
- edge 端末:
  Xperia 5 III
- host 端末:
  現在使用中の Windows PC `Tezy-GT37`
- 将来拡張:
  他 Android 端末、iOS 系端末、他 Windows PC、Ubuntu 単体 host へ拡張予定
...
- 一問一葉で 1 つの問いだけに集中できること
- テキスト、音声、写真が同じ入力文脈で扱えること
- 保存ボタン未押下でも、テキスト、音声、写真の変化が端末内へ自動で仮保存されること
- 同期待ちや認証待ちを意識させず、仮保存済みと同期段階が自然に分かること
- Xperia 5 III の片手操作でも数秒で入力を閉じられること
- 端末上でメモ一覧を sort 付きで見返せること
- 閲覧履歴から最近見たメモへ即座に戻れること
- 音声メモと画像メモにも文字見出しが付き、一覧で判別できること

### PC 側 UX

- 同期到着、解析中、解析済みが一覧で直感的に分かること
- project ごとの文脈、元ログ、次問、KPI 候補が同じ画面文脈で追えること
- 同期エラーや未解決項目が、管理者にとってすぐ対処できる粒度で見えること
- 現在の Windows host 上で、Docker / WSL2 状態を運用負荷なく観測できること

## 非交渉事項

- スマホ起動から入力完了まで数秒で到達できること
- 入力途中の断片メモが保存ボタン未押下でも端末内から失われないこと
...
- 現行の最適化対象は Xperia 5 III と `Tezy-GT37` の組み合わせとする
