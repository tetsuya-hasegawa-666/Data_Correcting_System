...
- PC 側:
  native EXE ではなく local web app。Desktop shortcut `iClone Host App` からアイコン起動できる。
- Android 側:
...
2. Desktop に `iClone Host App` `iClone Host Stack` `iClone Host App Stop` ができる
...
1. Desktop の `iClone Host Stack` をダブルクリックする
...
4. その後に Syncthing、Ollama、observer の骨格が起動する
5. 起動に失敗した場合は `runtime\logs\start_host_stack.log` を確認する

### 5. PC app を起動する

1. Desktop の `iClone Host App` をダブルクリックする
2. shortcut を使わない場合は次を実行する

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\start_host_app.ps1
```

3. ブラウザで `http://127.0.0.1:8874/preview/index.html` が開く

### 6. Android app を install する
...
1. 基本は Desktop の `iClone Host App` から開く
2. 直接見る場合は次を使う
