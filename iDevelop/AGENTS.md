...
このサブプロジェクトの運用規約の入口は `docs/index.md` である。
`docs/` は source-of-truth と運用規約を扱い、`develop/` は実開発の計画と履歴を扱う。

## Binding Rule

- このディレクトリはメインプロジェクトとは完全分離で扱う。
- ダッシュボードに関する文書、コード、設定、保持データ定義、結果整理、検証記録は `iDevelop/` 配下だけで扱う。
- companion project `iSensorium/` との境界は、各 project の `docs/index.md` と `develop/index.md` で管理する。
- 規約変更は `docs/` に反映する。
