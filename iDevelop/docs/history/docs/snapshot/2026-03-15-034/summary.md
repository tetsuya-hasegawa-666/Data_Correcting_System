...
- trigger: the user requested a new MRL to remove hover flicker, stabilize sticky headers, and clarify consultation versus draft actions
- resulting_direction: `current_state.md` now records `MRL-21` completion and click-controlled shell headers, while the plan set closes the paired consultation/draft action-box UX
- expected_benefit: the shell becomes easier to inspect without accidental expansion, and document work keeps consultation and draft responsibilities readable during scrolling

## 2026-03-15-033 consultation-draft-expression-alignment-line

- scope: dashboard documentation system
- trigger: the user requested a more human-readable and visually aligned explanation for consultation and draft
- resulting_direction: `current_state.md` now records `MRL-20` completion and the aligned card structure, while plan/history continue the release line sequence
- expected_benefit: users can compare consultation and draft quickly because both cards explain purpose, effect, and source impact in the same pattern

## 2026-03-15-032 preview-dual-column-action-line

- scope: dashboard documentation system
- trigger: the user requested a new MRL to split the preview into separate consultation and draft columns
- resulting_direction: `current_state.md` now records `MRL-19` completion and the dual-column preview rule, while develop planning/history track the new release line closure
- expected_benefit: the user can operate consultation and drafting in parallel without mixing the two action models in one block
