...
- `document-workspace` は search/preview に加え、edit/save を in-app save で扱う。
- document source policy は「seed bootstrap + localStorage save」で開始し、実ファイル同期は後続 line へ残す。
- `data-workspace` は dataset update と recent result summary を持つ。
- `shared-core` は document/data/code の workspace tab と error banner を持つ。
- `code-workspace` は read-only entry として static target list を表示し、実行・attach・編集は持たない。
