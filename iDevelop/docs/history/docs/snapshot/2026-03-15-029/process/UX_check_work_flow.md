...
4. code target を bundle に追加できることを確認する
5. focus input から code consultation を実行できることを確認する
6. `phase-gated-read-only` が shared shell に表示され、apply preview が出ないことを確認する

## Shared Shell Check

1. document consultation を 1 回実行する
2. data consultation を 1 回実行する
3. shared shell に current prompt、current bundle、summary、approval、history が表示されることを確認する
4. `Keep` `Discard` `Task化` `Apply Request` が表示されることを確認する

## Safe Apply Check

1. seed mode で document consultation を実行する
2. `Apply Request` を押して preview を表示する
3. `Approve` を押す
4. `apply approved` が表示され、document preview に `[Applied Consultation]` が追記されることを確認する
5. code consultation では `Apply Request` を押しても preview が出ないことを確認する
...
- shared shell で prompt、bundle、summary、history、proposal action が確認できる
- seed mode の document apply preview / approve が確認できる
