...
- session archive と export metadata を viewer が読める contract をそろえる
...
- `iDevelop` は `iSensorium` などの session/export 情報を viewer 用に読む
- viewer が最低限扱う field は次とする
  - `sessionId`
  - `recordingMode`
  - `requestedRoute`
  - `activeRoute`
  ...
  - `finalizedAt`
  - `files`
- 文書 explorer は top directory ごとの compact tree を基本とし、`.md` を含む directory を優先表示できるようにする
- download action は source path と export path の整合が確認できる範囲で one-click 化する
