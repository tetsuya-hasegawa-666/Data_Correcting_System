...
加えて、`iSensorium` などが出力した記録データと session 情報を、人が状況を把握しやすい形で閲覧し、カテゴリから探し、1 操作で取り出せる backend UI を提供する。
...
- `iDevelop` は記録データ閲覧、文書カテゴリ探索、ダウンロード導線を担う server-side frontend 相当 project でもある
...
- 日本語で自然に表現できる UI、説明、エラー文は極力日本語にする
- file 名、class 名、識別子、config key はアルファベットで維持する
- explorer、viewer、download、consultation の実装は MVC を崩さない
- 実装完了扱いは、原則として Codex 再テスト、例外としてユーザー明示の検証結果で行う
...
- session archive と文書カテゴリを compact に探索できる viewer 計画が定義されている
- one-click download と詳細 error handling が source-of-truth に定義されている
