...
- explorer、viewer、download、consultation の各系統で責務を混在させない。

## Error Handling Rules

- empty state、warning state、retry state、download failure、source mismatch、missing file を別々に扱う
- operator が次 action を判断しやすい日本語メッセージを優先する
- 失敗しても explorer、viewer、consultation の他機能を巻き込んで停止させない
...
- 完了根拠には `Codex retest` または `user validation` を明記する。
