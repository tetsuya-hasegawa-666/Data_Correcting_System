...
1. `mRL-2-2` は `data-workspace` の browse/aggregate/chart slice として実装済みである。
2. `mRL-2-3` は static dataset seed contract を `datasets.json` と controller 集計へ固定することで完了した。
3. 次の target micro release は `mRL-3-1` であり、optional scope の code/debug workspace を今回の後続フェーズへ入れるか判断する。
