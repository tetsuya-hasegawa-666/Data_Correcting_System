...
- `MRL-2` の must scope line は、`document-workspace` と `data-workspace` の最小 slice 実装により成立した。
- 次に着手する market release line は `MRL-3` であり、optional scope の code/debug workspace を phase gate 判定する。
- この turn では `MRL-3` には入らず、must scope 成立時点で停止する。
