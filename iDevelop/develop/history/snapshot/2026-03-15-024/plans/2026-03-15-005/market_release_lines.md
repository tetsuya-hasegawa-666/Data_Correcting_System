...
| MRL-12 | Data Consultation Line | データを選んで Codex に相談し、要約・確認ポイント・次 action を得られる | completed |
| MRL-13 | Shared Conversation Shell Line | document/data を横断する共通 conversation shell を提供する | completed |
| MRL-14 | Code Consultation Phase-Gate Line | code を consultation material として read-only 参照できる | completed |
| MRL-15 | Proposal-To-Action Line | Codex 提案を keep / discard / task 化できる | completed |
| MRL-16 | Safe Apply Line | user approval 付きの限定 apply flow を document/data に導入する | completed |
| MRL-17 | Pilot Interaction Line | 選ぶ -> 相談する -> 判断する -> 記録する の end-to-end UX を閉じる | completed |
