...
| mRL-1-3 | MRL-1 | edit/save 追加後も既存検索が壊れず、UI で操作確認できる | 回帰 test、build、`npm run dashboard` による exploratory check、`UX_check_work_flow.md` 更新を確認する | document-workspace の最小 authoring slice が成立する | search regression | source sync |
...
| mRL-2-3 | MRL-2 | source-of-truth と seed の衝突条件を見落とさない | current state、protocol、`UX_check_work_flow.md` を確認する | conflict handling と rollback 観点が残る | stale overwrite | sync safeguards |
...
| mRL-3-3 | MRL-3 | data workflow 追加後も metric 表示が壊れず、UI で追える | 回帰 test、build、exploratory check、`UX_check_work_flow.md` 更新を確認する | data-workspace の workflow slice が成立する | chart regression | shared state |
| mRL-4-1 | MRL-4 | document/data の navigation を共通化できる | shared-core の contract test と `UX_check_work_flow.md` を確認する | workspace 切替 contract と entry 導線が決まる | routing drift | shell navigation |
...
| mRL-5-2 | MRL-5 | 採用する場合の最初の read-only story を 1 つに絞り、体験手順に反映できる | BDD story と `UX_check_work_flow.md` を確認する | file browse または log preview のどちらかに限定される | mixed optional scope | optional slice |
| mRL-6-1 | MRL-6 | 次フェーズの first target を迷わず選べる | current recommendation を確認する | 継続順序が固定される | scattered backlog | handoff note |
| mRL-6-2 | MRL-6 | 維持すべき回帰観点と UI 操作確認観点を一覧できる | `npm test`、`npm run build`、`npm run dashboard`、manual UI check、`UX_check_work_flow.md` を確認する | regression checklist が残る | hidden regression | verification pack |
| mRL-6-3 | MRL-6 | 残課題と defer 理由を短く引き継げる | current state、履歴、`UX_check_work_flow.md` を確認する | entry command を含む次フェーズ handoff が成立する | missing context | new plan set |

## Current Recommendation

1. `mRL-1-1` から `mRL-5-2` までは、edit/save、source policy、data update、shared navigation、read-only code entry の実装で満たした。
2. `mRL-6-1` は entry command と継続順序の固定で満たした。
3. 現在残る target は `mRL-6-2` と `mRL-6-3` で、manual UI exploratory check の記録と handoff の最終整理である。
