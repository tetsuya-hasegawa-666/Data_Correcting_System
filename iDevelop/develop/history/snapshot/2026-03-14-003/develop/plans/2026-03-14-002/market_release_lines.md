...
- `MRL-0` から `MRL-4` までの kickoff 条件は、`document-workspace` first slice の確定により満たした。
- 最初の実装 slice は `shared-core` bootstrap + `document-workspace` の一覧・検索・プレビューとする。
- 次の継続対象は `MRL-2` 配下の `mRL-2-2` と `mRL-2-3`、すなわち data workspace の BDD/TDD 入口である。
