...
1. `mRL-4-1` の first implementation slice は `document-workspace` の一覧・検索・プレビューに確定した。
2. `mRL-4-2` の first TDD order は `DocumentWorkspaceController` と `DocumentWorkspaceView` の failing test から開始し、green を確認済みである。
3. 次の target micro release は `mRL-2-2` とし、data workspace の browse/aggregate/chart の最初の BDD ストーリーを選ぶ。
