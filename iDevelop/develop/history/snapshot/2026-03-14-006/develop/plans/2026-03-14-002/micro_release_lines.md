...
1. `mRL-4-1` は次セッションの first target を `document-workspace` edit/save slice に固定することで完了した。
2. `mRL-4-2` は failing test の開始点を controller の edit intent と save action に絞ることで完了した。
3. `mRL-4-3` は停止条件を「edit/save の最小 green と履歴更新まで」に固定することで完了した。
