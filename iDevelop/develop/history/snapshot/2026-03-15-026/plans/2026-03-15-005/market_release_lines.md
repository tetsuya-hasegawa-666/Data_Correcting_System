...
`2026-03-15-005` drives `iDevelop` from consultation-first contract work to a usable consultation workspace with explicit editing safety and compact shell UX.
...
- `MRL-18` closes the current UX clarification line on top of the completed consultation baseline
- document search now runs as a left explorer pane with preview-side action decisions
- read-only unlock is now framed as `local draft` before any safe apply step

## Current Recommendation

- `2026-03-15-005` is completed through `MRL-18`
