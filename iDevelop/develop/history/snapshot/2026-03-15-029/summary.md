...
## 2026-03-15-029 header-and-action-box-stabilization-implementation

- target_behavior: shell headers should expand only on click, and document preview should expose consultation and draft as paired action boxes with sticky local headers
- intended_change: define and close `MRL-21` with click-controlled header cards, consultation composer save/cancel flow, and mirrored preview action boxes
- background_reason: the user reported hover-driven flicker, unclear action affordances, and scrolling that buried the local document controls
- change_summary: rewired dashboard shell interaction, rebuilt document preview action structure, extended controller/view tests, and closed the release line with green test/build verification
- affected_documents: `develop/history/summary/summary.md`, `develop/plans/2026-03-15-005/market_release_lines.md`, `develop/plans/2026-03-15-005/micro_release_lines.md`
- expected_effect: document consultation/editing feels more stable in operation, and the top shell no longer distracts or hides detail behind accidental hover

## 2026-03-15-028 consultation-draft-expression-alignment-implementation

- target_behavior: consultation and draft should look equally intentional and be comparably readable
- intended_change: define and close `MRL-20` with aligned preview card structure and clearer user-facing wording
- background_reason: the user reported that the two columns did not yet feel visually or semantically aligned
- change_summary: updated document preview card copy and styling, extended tests for aligned structure, and closed the new release line with green verification
- affected_documents: `develop/history/summary/summary.md`, `develop/plans/2026-03-15-005/market_release_lines.md`, `develop/plans/2026-03-15-005/micro_release_lines.md`
- expected_effect: consultation and draft are easier to compare at a glance, reducing interpretation cost during document work

## 2026-03-15-027 preview-dual-column-action-implementation

- target_behavior: the preview should separate consultation work from draft work into two visible columns
- intended_change: define and close `MRL-19` with consultation-column and draft-column preview structure
- background_reason: the user requested easier操作 by splitting preview actions rather than stacking them in one section
- change_summary: updated document preview rendering and layout CSS, added tests for split columns, and closed the new release line with green verification
- affected_documents: `develop/history/summary/summary.md`, `develop/plans/2026-03-15-005/market_release_lines.md`, `develop/plans/2026-03-15-005/micro_release_lines.md`
- expected_effect: consultation and draft actions are clearer, safer, and faster to reach inside the preview context

## 2026-03-15-026 document-explorer-rebalance-implementation

- target_behavior: document search should use a narrow explorer pane while preview owns draft/unlock and consultation action choices
- intended_change: define and close `MRL-18` with explorer layout, preview-side action guidance, and overflow-safe tree rendering
- background_reason: the user reported that unlock and draft decisions were placed in the wrong pane, the search area had become too wide, and the flat grouped list did not behave like an explorer
- change_summary: rewired document workspace rendering into explorer + preview, clarified `相談` vs `Draft`, updated plans/current state/history, and closed the new release line with green tests/build
- affected_documents: `develop/history/summary/summary.md`, `develop/plans/2026-03-15-005/market_release_lines.md`, `develop/plans/2026-03-15-005/micro_release_lines.md`
- expected_effect: document navigation is denser and clearer, while editing decisions happen where the user can see the active preview context

## 2026-03-15-025 local-draft-ux-implementation

- target_behavior: completed consultation workspace remains operable when document/data sources begin read-only and the top shell must stay compact without hiding essential status
- intended_change: add local draft unlock overlays, consultation capability guidance, compact expandable header cards, and overflow-safe preview/table layout
- background_reason: the user reported ambiguous unlock conditions, weak discoverability for consultation output, oversized header height, and preview overflow beyond the right edge
- change_summary: introduced unlockable document/data repositories, updated dashboard shell rendering, refreshed document/data workspace guidance, hardened layout CSS, and extended tests for unlock/header behavior
- affected_documents: `develop/history/summary/summary.md`
- expected_effect: editing becomes available through explicit local draft unlock, consultation expectations are clearer, and the shell remains readable without clipping content

## 2026-03-15-024 shared-shell-to-pilot-implementation

- target_behavior: `MRL-13` から `MRL-17` までを completed 状態で閉じる
- intended_change: shared shell、code phase gate、proposal action、safe apply、pilot closure を実装事実に合わせて current state と plan に反映する
- background_reason: document/data consultation まで green になったため、残りの shared-core centered lines を同一 contract 上で閉じられる段階に達したため
- change_summary: shared conversation shell、code consultation gate、proposal action、seed-mode safe apply を実装し、plan set `2026-03-15-005` を completed 状態へ更新した
- affected_documents: `develop/history/summary/summary.md`, `develop/plans/2026-03-15-005/market_release_lines.md`, `develop/plans/2026-03-15-005/micro_release_lines.md`
- expected_effect: 次の plan set は completed baseline から export / multi-user / deeper automation のいずれかへ進める

## 2026-03-15-023 data-consultation-implementation

- target_behavior: `MRL-12` を data consultation line として閉じる
- intended_change: dataset bundle selection、focus input、response panel を data workspace に追加し、`MRL-13` へ進める
- background_reason: document consultation と同じ contract shape を data にも通さないと shared shell を実装できなかったため
- change_summary: data workspace に consultation bundle / focus / response state を追加し、dashboard shell から data consultation を実行できるようにして、UX check と current state を `MRL-13 / mRL-13-1` へ更新した
- affected_documents: `develop/history/summary/summary.md`, `develop/plans/2026-03-15-005/market_release_lines.md`, `develop/plans/2026-03-15-005/micro_release_lines.md`
- expected_effect: shared conversation shell を document/data 共通 state で実装開始できる

## 2026-03-15-021 document-consultation-implementation

- target_behavior: `MRL-11` を document consultation line として閉じる
- intended_change: multi-document bundle selection、focus input、response panel を document workspace に追加し、`MRL-12` へ進める
- background_reason: interaction contract は fixed になったため、must scope の先頭である document consultation を green にしないと shared shell へ進めなかったため
- change_summary: document workspace に consultation bundle / focus / response state を追加し、dashboard shell から document consultation を実行できるようにして、UX check と current state を `MRL-12 / mRL-12-1` へ更新した
- affected_documents: `develop/history/summary/summary.md`, `develop/plans/2026-03-15-005/market_release_lines.md`, `develop/plans/2026-03-15-005/micro_release_lines.md`
- expected_effect: data consultation を document line と同じ contract shape で実装開始できる

## 2026-03-15-022 continuation-window-rule

- target_behavior: active な market release line を 15 分前後で中断せずに進められる
- intended_change: active MRL は user block または 6 時間上限まで継続する規約を docs/develop と active plan set に反映する
- background_reason: handoff / stop 条件寄りの記述が残っていると、`MRL-12` の途中でも短時間で停止しやすく、exit criteria 到達前に作業が切れやすい
- change_summary: `AGENTS.md`、`research_operation.md`、`current_state.md`、`develop/index.md`、`2026-03-15-005` の market/micro release line に continuation window rule を追加した
- affected_documents: `develop/history/summary/summary.md`, `develop/index.md`, `develop/plans/2026-03-15-005/market_release_lines.md`, `develop/plans/2026-03-15-005/micro_release_lines.md`
- expected_effect: `MRL-12` の中では `mRL-12-1` 完了後に `mRL-12-2`、`mRL-12-3` へ続けて進みやすくなり、15 分前後での任意停止を避けやすくなる

## 2026-03-15-020 interaction-contract-implementation

- target_behavior: `MRL-10` を interaction contract line として閉じる
- intended_change: consultation session model、contract 表示、`MRL-10` 用 BDD/TDD 定義を実装と文書へ反映する
- background_reason: `2026-03-15-005` は consultation-first に再定義済みだが、bundle / response / approval state が未固定のままでは `MRL-11` に入れなかったため
- change_summary: `shared-core/model/ConsultationSession` と dashboard shell の contract 表示を追加し、source-of-truth を `bundle` `summary` `evidence` `next_action` `approval state` に固定して current target を `MRL-11 / mRL-11-1` へ進めた
- affected_documents: `develop/history/summary/summary.md`, `develop/plans/2026-03-15-005/market_release_lines.md`, `develop/plans/2026-03-15-005/micro_release_lines.md`
- expected_effect: 次の target を document consultation の failing test へ接続し、shared shell と safe apply の前提 schema を安定化できる

## 2026-03-15-019 consultation-workspace-plan-set

- target_behavior: `iDevelop` を user-Codex consultation workspace として再始動できる
- intended_change: plan set `2026-03-15-005` を作成し、`MRL-10` から `MRL-17` まで consultation-first な release line を定義する
- background_reason: `2026-03-14-004` で read-only dashboard と pilot 基盤はそろったが、本来価値であるユーザー相談体験は未実装のため
- change_summary: interaction contract、document/data consultation、shared conversation shell、code phase gate、proposal-to-action、safe apply、pilot interaction を market/micro line に分解した
- affected_documents: `develop/history/summary/summary.md`, `develop/plans/2026-03-15-005/market_release_lines.md`, `develop/plans/2026-03-15-005/micro_release_lines.md`
- expected_effect: 次の実装セッションで `MRL-10 / mRL-10-1` から迷わず consultation contract 定義へ着手できる
