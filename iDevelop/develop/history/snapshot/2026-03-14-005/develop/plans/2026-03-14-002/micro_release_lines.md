...
1. `mRL-3-1` は must scope 優先の観点から、`code-workspace` を今回のラインへ入れない判断で完了した。
2. `mRL-3-2` は read-only path 制約、no process attach、debug control defer を安全境界として固定することで完了した。
3. 次の target micro release は `mRL-4-1` であり、handoff 用に次の実装対象を 1 つへ絞る。
