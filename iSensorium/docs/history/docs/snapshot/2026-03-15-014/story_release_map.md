...
- `MRL-10 guarded replacement preview MVP` は、shared camera capture session の target を増やさず、recorder に流れる保存用フレームの複製から `640x480` 系 `5fps` 始動の live preview を作る line とする
- `MRL-7` 以降のマイクリリース焦点:
