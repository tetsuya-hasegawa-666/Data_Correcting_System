...
- 次:
  - 次の作業は利用者による実 UX 確認である。
  - UX 確認結果に応じて `採用 / guarded 継続 / rollback` のいずれかへ分岐する。
- blocker:
  - `MRL-8` と `MRL-9` 自体の blocker はない。
  - 次の blocker は利用者 UX 確認で観測される実機依存の UX 差分のみである。
- 検証条件:
...
- thread purpose: `利用者が guarded replacement route を選び、UX check を開始できる ready 状態を維持する`
