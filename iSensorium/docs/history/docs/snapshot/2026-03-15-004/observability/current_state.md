...
- active plan set candidate: `develop/plans/2026-03-15-004/`
- active market release target: `MRL-6 guarded upstream trial`
- active micro release target: `mRL-6-1 adapter seam with frozen-route fallback`
...
- next:
  - keep the frozen route as the default runtime path.
  - verify that downstream parsing still accepts additive manifest fields only.
  - wire the actual replacement runtime only after the guarded seam proves reversible.
- blocker:
  - `coreCamera` replacement runtime is validated in the sibling project, but not yet imported into `iSensorium` runtime behind the seam.
- validation condition:
  - enabling `BuildConfig.CAMERA_STACK_ROUTE = "corecamera_shared_camera_trial"` must not be treated as integration-ready until the active route stops falling back to `frozen_camerax_arcore`.
