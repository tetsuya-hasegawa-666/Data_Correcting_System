...
| Preparation | `USER_PREPARATION.md` | 利用者準備、外部 device access、手動前提の単一ノート | user action を書く場所、記載対象の境界 | 現在必要な外部準備、解消済み事項 |
...
4. 利用者準備、外部 device access、手動前提は `USER_PREPARATION.md` を先に更新する。
...
- 利用者準備、外部 device access、手動前提は `USER_PREPARATION.md` だけに置き、`docs/process/UX_check_work_flow.md` は参照だけを持つ。
