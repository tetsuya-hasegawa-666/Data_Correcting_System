...
## 2026-03-15-006 user-preparation-note-restored

- scope: documentation system
- trigger: workspace 共通ルールで user action / external device access を `USER_PREPARATION.md` に集約する方針へ戻ったため、`iSensorium` の live 文書も現行ルールへ再適合する必要が出た
- resulting_direction: `USER_PREPARATION.md` を source-of-truth として復活させ、`docs/index.md`、`README.md`、`docs/process/UX_check_work_flow.md`、関連 process 文書を「準備ノート参照 + UX 手順本体」の境界へ整理した
- expected_benefit: 利用者準備の更新先が一意になり、今後の guarded upstream trial で user-side task と UX 評価手順が混線しにくくなる
...
- scope: documentation system
- trigger: adapter seam 実装、parser compatibility check、reversible cutover gate の 3 点が揃い、`MRL-6` の exit 条件を満たした
- resulting_direction: `current_state` と `story_release_map` を `MRL-6 completed` として更新し、次は実 runtime wiring 用の新 dated plan set を起こす前提に整理した
- expected_benefit: guarded preparation line と実 integration line が混ざらず、次の変更は replacement runtime wiring に限定できる
...
- scope: documentation system
- trigger: reproducible evidence showed `ARCore ON` causes multi-second camera-path stalls while `IMU` remains stable
- resulting_direction: freeze the current `CameraX + ARCore` mainline, preserve the findings as restart-ready handover, and open an isolated sibling project `coreCamera/` for `Camera2 + Shared Camera` replacement work
- expected_benefit: the next session can start directly on the correct camera architecture without re-litigating the failure cause

## 2026-03-15-001 continuation-interpretation-hardening
