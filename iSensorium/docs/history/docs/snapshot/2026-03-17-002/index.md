...
- `iSensorium/` は機能全体のうち、Android 端末上の計測、同期記録、session metadata、export 契約を担うバックエンド相当 project として扱う
- `iDevelop/` は取得済みデータと文書を閲覧、整理、取得する server-side frontend 相当 project として扱う
- 実装作業は、Android 計測系なら `iSensorium/`、閲覧・取得 UI 系なら `iDevelop/` の配下で行う
- どちらへ置くべきか即断できない変更だけを相談対象とする
...
12. MVC、詳細エラーハンドリング、日本語 UI 方針、再テスト方針のような全体実装原則は、実装前に source-of-truth へ先に反映する。
...
- Android 画面で日本語が自然に使える箇所は極力日本語にする
- file 名、class 名、package 名、識別子、API 名、config key はアルファベットを維持する
- `iSensorium/` の実装は MVC に準拠し、View、Controller、Model の責務を混在させない
- recorder、session、export、UI それぞれで詳細なエラーハンドリングを持たせる
- 実装完了扱いは、Codex の再テスト通過後、またはユーザーが明示した検証結果を完了根拠として採用した後に限る
