...
- Android 端末上の計測、同期記録、session metadata、export contract は `iSensorium/` 配下で扱う
- 閲覧、カテゴリ探索、ダウンロード、server-side frontend としての viewer UX は `iDevelop/` 配下で扱う
- どちらへ置くべきか判断できない変更だけを相談対象とし、それまでは担当 project 内で継続する
...
- ただし、ユーザーが検証済みであることを明示した場合は、そのユーザー検証を完了根拠として採用できる
