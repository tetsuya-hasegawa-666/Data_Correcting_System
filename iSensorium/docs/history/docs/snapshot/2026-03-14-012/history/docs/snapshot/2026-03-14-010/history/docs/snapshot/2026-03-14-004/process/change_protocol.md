...
文書体系そのものを変更した場合は、個別差分の羅列ではなく、文書体系全体がどの方向へ動いたかを短く要約して `docs/history/docs/summary/` に 1 件追加する。
また、価値検証と効果検証のため、同じ変更時点の `docs/` 全体スナップショットを `docs/history/docs/snapshot/` に保存する。
...
要約履歴は「何が変わったか」より「この変更で文書体系全体がどの方向へ寄ったか」を 2-5 行で記録する。

## Docs Pair Rule

当面の運用では、文書体系変更時に以下を必須とする。

1. `docs/history/docs/summary/` に短い要約履歴を 1 件追加する。
...
実開発変更時は、`docs/history/develop/summary/` の履歴と対になる形で、`docs/history/develop/snapshot/` に「変更した文書のみ」のスナップショットを保存する。
