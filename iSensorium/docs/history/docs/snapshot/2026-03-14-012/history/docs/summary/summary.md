...
- scope: documentation system
- trigger: GNSS と BLE の実機統合が進み、ARCore だけが構成制約で残る状態になった
- resulting_direction: source-of-truth に `MRL-2` の進捗を反映し、拡張センサ統合のうち「追記で済むもの」と「camera/GL 経路変更が必要なもの」を分離した。
- expected_benefit: 次の実装で ARCore だけに焦点を当てられ、`MRL-3` へ早すぎる移行を防げる。

## 2026-03-14-011 mrl2-completed-with-gl-thread-arcore

- scope: documentation system
- trigger: ARCore を GL thread 上へ移して、動画 + IMU + GNSS + BLE + ARCore の複合 session が実機で成立した
- resulting_direction: `MRL-2` を完了扱いに更新し、次の関心を「センサ統合」から「後段で読める schema / parser contract」へ移した。
- expected_benefit: `MRL-3` への移行条件が明確になり、これ以上 `MRL-2` へ戻る理由と残課題を切り分けやすくなる。

## 2026-03-14-012 mrl3-parser-contract-established

- scope: documentation system
- trigger: Python parser と validation report で session 再利用性を確認できた
- resulting_direction: source-of-truth を「記録できる」状態から「後段で読める」状態へ進め、join key と parser contract を明示した。
- expected_benefit: `MRL-4` 以降の運用試験でも、回収した session を同じ読み取り contract で再検証できる。
