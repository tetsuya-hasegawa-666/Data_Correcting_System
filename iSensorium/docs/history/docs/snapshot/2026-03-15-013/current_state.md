...
- 次:
  - 保存アウトプットは想定どおりと確認できたため、次の作業は残る UX blocker の解消である。
...
- guarded replacement route は「保存アウトプット整合あり、toggle crash 解消、preview live は未解決」という前提で扱う。
- shared camera capture session へ追加 preview surface を直接足す案は Xperia 5 III では不成立だったため、次の preview 対策は別経路で検討する。
