...
- replacement route recording 中に preview が動かない点は未解決
- shared camera capture session に `TextureView` preview surface を足す案は、Xperia 5 III で `Failed to create capture session; configuration failed` を再現したため採用していない
 - recorder 側フレーム複製から `640x480` 系 `5fps` 始動の preview MVP 実装は完了しており、実機確認待ち
...
- replacement route recording 中の live preview はまだ提供していない
- Xperia 5 III では camera2 shared camera session に preview surface を追加する方式は構成失敗したため、別経路の preview 対策が必要である
