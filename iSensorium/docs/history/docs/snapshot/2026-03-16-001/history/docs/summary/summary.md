...
## 2026-03-16-001 MRL-10 の Codex 再検証証跡追加

- 対象範囲: 文書体系
- 発生理由: `MRL-10` は利用者実機確認で閉じていたが、Codex 自身の build / test / device evidence を追加で残してから次テーマへ進む必要が出た
- 反映内容: `current_state.md` に `./gradlew.bat clean assembleDebug testDebugUnitTest`、`python -m unittest python.test_session_parser`、Xperia 5 III 実機 session `session-20260316-200652` の再検証結果を追加した
- 期待効果: 次のテーマでは `MRL-10` の完了根拠として、利用者確認だけでなく Codex 自身の再検証証跡も source-of-truth から参照できる

## 2026-03-15-016 MRL-10 preview MVP 完了

- 対象範囲: 文書体系
- 発生理由: guarded replacement route で recorder side preview tap による live preview MVP が実機確認まで完了し、`MRL-10` を閉じる条件が揃った
- 反映内容: `current_state.md`、`system_blueprint.md`、`story_release_map.md`、`UX_check_work_flow.md`、`develop/index.md`、`develop/plans/2026-03-15-014/` を `MRL-10 completed` 状態へ更新した
- 期待効果: 次の会話では preview 未解決を前提にせず、新しい dated plan set の定義か、preview MVP 後続改善の判断から開始できる
...
- 対象レイヤ: source-of-truth
- 変更要約: `MRL-10 guarded replacement preview MVP` を定義し、shared camera capture session の target を増やさず recorder 側フレーム複製から `640x480` 系 `5fps` 始動 preview を作る方針を active target として固定した
- 判断根拠: Xperia 5 III で preview surface 追加案が `Failed to create capture session; configuration failed` となったため、別経路 preview が必要になった
- 影響: guarded replacement route は recording 成立と保存整合を維持したまま preview live 追加へ進む

## 2026-03-15-015 MRL-10 preview MVP 実装反映

- 対象レイヤ: source-of-truth
- 変更要約: `mRL-10-1` と `mRL-10-2` の実装完了を current state と UX workflow に反映し、実機確認対象を `mRL-10-3` に進めた
- 判断根拠: recorder 側フレーム複製から `640x480` 系 `5fps` preview を UI に出す実装が完了したため
- 影響: guarded replacement route は preview MVP を実機確認する段階に進んだ
