...
| RL-7 | in_progress | actual replacement runtime wiring を `iSensorium` で開始する |
...
1. `MRL-7` を先に閉じ、replacement runtime を seam の裏へ最小差分で接続する。
2. `MRL-8` では preview / stop / refresh / rollback を guarded 条件内で安定化する。
3. `MRL-9` で user UX check 用文書と観察点を確定し、実確認へ渡す。
