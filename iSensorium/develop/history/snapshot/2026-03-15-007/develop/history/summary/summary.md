...
## 2026-03-15-006 user-preparation-doc-conformance

- target_behavior: `iSensorium` の live markdown を workspace 共通の user preparation rule に再適合させる
- intended_change: `USER_PREPARATION.md` を復活し、`README.md` と `docs/process/*` を準備ノート参照前提へ組み替える
- background_reason: 直近の `iSensorium` 文書は `UX_check_work_flow.md` に利用者準備を内包していたが、workspace 共通ルールでは `USER_PREPARATION.md` を source-of-truth とする
- change_summary: added `USER_PREPARATION.md`, updated `docs/index.md`, `docs/process/change_protocol.md`, `docs/process/research_operation.md`, `docs/process/UX_check_work_flow.md`, and `README.md` to separate user preparation from UX evaluation flow
- affected_documents: `USER_PREPARATION.md`, `README.md`, `docs/index.md`, `docs/process/change_protocol.md`, `docs/process/research_operation.md`, `docs/process/UX_check_work_flow.md`, `docs/history/docs/summary/summary.md`, `develop/history/summary/summary.md`
- expected_effect: future user-side preparation updates land in one file, while UX check instructions stay focused on evaluation steps and rollback notice
...
## 2026-03-15-005 guarded-upstream-trial-line-completed

- target_behavior: close `MRL-6` as a completed preparation line before any real replacement runtime wiring begins
- intended_change: add explicit gate status to route resolution, verify parser compatibility with additive manifest fields, and mark the dated plan set as preparation-complete
- background_reason: without a formal close-out, the preparation line and the actual runtime integration line would blur together and weaken rollback clarity
- change_summary: added `CORECAMERA_RUNTIME_ENABLED`, expanded route metadata with `cutoverGateStatus`, added Python parser regression coverage, and updated docs/develop state to show `MRL-6` complete
- affected_documents: `app/build.gradle.kts`, `app/src/main/java/com/isensorium/app/GuardedUpstreamTrial.kt`, `app/src/main/java/com/isensorium/app/MainActivity.kt`, `app/src/test/java/com/isensorium/app/GuardedUpstreamTrialContractTest.kt`, `python/test_session_parser.py`, `docs/artifact/system_blueprint.md`, `docs/artifact/story_release_map.md`, `docs/observability/current_state.md`, `docs/history/docs/summary/summary.md`, `develop/index.md`, `develop/history/summary/summary.md`
- expected_effect: the next dated plan set can focus only on importing and validating the replacement runtime behind the already-verified seam
