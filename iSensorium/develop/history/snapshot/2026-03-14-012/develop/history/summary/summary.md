...
- target_behavior: `mRL-3-1`, `mRL-3-2`, `mRL-3-3` の完了
- intended_change: session directory を Python で読み込み、join key と metadata sufficiency を検証できる parser / validator を追加する
- background_reason: `MRL-3` では、実機で取れた session を後段処理へ再利用できることを local parser で確認する必要があった
- change_summary: `python/session_parser.py` と `python/validate_session.py` を追加し、`tmp/session-20260314-111930/validation_report.json` で stream count、time range、join delta、metadata sufficiency を確認した
...
- expected_effect: `MRL-3` が完了し、次は屋外運用・発熱・partial failure 回収を扱う `MRL-4` へ進める
