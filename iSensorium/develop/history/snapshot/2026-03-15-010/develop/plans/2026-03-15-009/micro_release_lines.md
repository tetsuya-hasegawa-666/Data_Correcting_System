...
- `mRL-7-1`: `CoreCameraTrialSessionAdapter`、`TrialSharedCameraLifecycleMachine`、offscreen ARCore pose sampler、CPU image video recorder を `iSensorium` 内へ追加し、requested replacement route が actual runtime を起動できるようにした。
- `mRL-7-2`: `session_manifest.json`、`video.mp4`、`imu.csv`、`gnss.csv`、`ble_scan.jsonl`、`arcore_pose.jsonl`、`video_frame_timestamps.csv`、`video_events.jsonl` の contract を維持し、`python -m unittest python.test_session_parser` を再通過した。
- `mRL-7-3`: `CORECAMERA_RUNTIME_ENABLED = true` で guarded activation を有効化しつつ default route は frozen のまま維持し、`./gradlew.bat clean assembleDebug testDebugUnitTest` を通過した。
