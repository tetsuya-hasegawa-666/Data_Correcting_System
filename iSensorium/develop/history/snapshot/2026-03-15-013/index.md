...
- active dated plan set: `2026-03-15-011 MRL-8 guarded upstream stabilization + MRL-9 user UX check ready`
...
2026-03-15 の defect recovery では、shared camera capture session へ preview surface を直接追加する案も試したが、Xperia 5 III で `Failed to create capture session; configuration failed` を再現したため撤回した。現行実装は recording 成立優先の最小復旧状態で維持する。
