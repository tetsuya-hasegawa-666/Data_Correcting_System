...
通常計測とポケット収納計測の両 mode を、同じ session contract 上で確認する。
...
ポケット収納計測を試す場合は、`記録モード` でポケット収納向け mode を選んでから開始する。
...
- `Use guarded replacement route` の短間隔 toggle crash は解消済み
- shared camera capture session に `TextureView` preview surface を足す案は、Xperia 5 III で `Failed to create capture session; configuration failed` を再現したため採用していない
- recorder 側フレーム複製から `640x480` 系 `5fps` 始動の preview MVP を実装し、実機で preview 更新まで確認済み
- preview は保存用フレーム複製を best-effort 表示するため、高精細 preview ではなく MVP 品質として扱う
...
7. 日本語化された画面文言が、操作の意味を自然に伝えるか
8. `記録モード` の違いが UI と session 結果の両方で理解できるか
9. ポケット収納向け mode でも、誤操作しにくく状態が把握しやすいか

## 2026-03-15 現在の制約

- guarded replacement route は recording 成立と保存アウトプット整合を優先している
- replacement route recording 中の live preview は recorder 側フレーム複製ベースの MVP であり、`640x480` 系 `5fps` 始動の best-effort 表示である
- Xperia 5 III では camera2 shared camera session に preview surface を追加する方式は構成失敗したため、別経路 preview を採用している
...
- Codex が完了扱いを更新する場合は、原則として Codex 自身の再テストを先に行う
- ただし、ユーザーが自分の検証結果を明示した場合は、その user validation を完了根拠として採用できる
- どちらを採用したかは `current_state.md` に記録する
