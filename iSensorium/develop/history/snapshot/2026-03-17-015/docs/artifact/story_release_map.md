...
## 2026-03-17 追補: 通常計測 / ポケット収納計測 と閲覧導線

- 新しい利用者アクティビティ:
  - `User Activity: 携行状況に応じて記録モードを選ぶ`
  - `User Activity: 取得済み session と文書をカテゴリから探して確認する`
- 新しいユーザーストーリー:
  - `User Story: 通常操作中でもポケット収納中でも、迷わず適切な記録モードを選びたい`
  - `User Story: 取得済みデータと文書を、カテゴリごとに見やすく確認し、1 操作で取り出したい`
- 新しいシステム能力:
  - `Capability: mode-aware recording`
  - `Capability: archive exploration and one-click retrieval`
- 新しい変更ポイント:
  - `Change Point: 携行パターン差分`
  - `Change Point: session/export contract と viewer UI の整合`
- 次段の市場リリースライン:
  - `MRL-11 capture mode and contract line`
  - `MRL-12 mobile MVC and Japanese UX line`
  - `MRL-13 robust recording and error handling line`
- 次段のマイクリリース焦点:
  - `mRL-11-1`: 通常計測 / ポケット収納計測 mode を同一命名で定義する
  - `mRL-11-2`: session manifest と export contract に mode metadata を追加する
  - `mRL-11-3`: docs / current state / develop を新 mode 前提へ更新する
  - `mRL-12-1`: Android UI で日本語化可能箇所を整理する
  - `mRL-12-2`: mode selection UI と controller 境界を追加する
  - `mRL-12-3`: MVC 責務整理と UI 再テスト条件を固定する
  - `mRL-13-1`: session / recorder / export の error handling を強化する
  - `mRL-13-2`: pocket mode の fallback、warning、retry 導線を整える
  - `mRL-13-3`: Codex 再テストまたはユーザー検証採用を含めて line を閉じる
