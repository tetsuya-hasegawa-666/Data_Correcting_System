...
- active dated plan set: `2026-03-17-015 capture mode, japanese UX, and resilient recording`
- latest completed plan set: `2026-03-15-014 MRL-10 guarded replacement preview MVP`
- active market release target: `MRL-11 capture mode and contract line`
- latest completed market release target: `MRL-10 guarded replacement preview MVP`
- active micro release target: `mRL-11-1 standard and pocket mode definition`
...
- 解釈:
  ...
  - `2026-03-15-009` は、その seam の裏へ replacement runtime を guarded に接続し、`MRL-7` を完了させた記録である。
  - `2026-03-15-011` は、guarded runtime の stabilization と user UX check ready を完了させた記録である。
  - `2026-03-15-014` は、guarded replacement route の preview MVP を recorder side tap 方式で完了させた記録である。
  - `2026-03-17-015` は、通常計測 / ポケット収納計測の mode 契約、Android 日本語 UX、MVC、error handling、viewer 連携前提を固定する新 active line である。
- 強い guardrail:
...
現在の基準状態では、default route は `frozen_camerax_arcore` のまま維持しつつ、app 内の `Use guarded replacement route` switch を ON にすると requested route `corecamera_shared_camera_trial` を選べる。`CORECAMERA_RUNTIME_ENABLED = true` のため、その組合せで guarded replacement runtime が起動する。

実機確認では、replacement route session の required artifact と manifest file size 整合、`Use guarded replacement route` の短間隔 toggle crash 解消、replacement route recording 中の live preview 更新まで確認済みである。`MRL-10` は閉じており、次の実装は新しい dated plan set 定義待ちである。

2026-03-15 の defect recovery では、shared camera capture session へ preview surface を直接追加する案も試したが、Xperia 5 III で `Failed to create capture session; configuration failed` を再現したため撤回した。`MRL-10` では shared camera capture session の target を増やさず、`TrialCpuImageVideoRecorder` に流れる保存用フレームから `640x480` 系 `5fps` 始動の preview MVP を作り、Xperia 5 III 実機で preview 更新まで確認した。
