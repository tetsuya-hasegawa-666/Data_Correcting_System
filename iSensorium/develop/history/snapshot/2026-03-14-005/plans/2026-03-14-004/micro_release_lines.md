...
|---|---|---|---|---|---|---|
| mRL-0-1 | MRL-0 | ルート直下に `iDevelop/` が存在することを確認できる | ディレクトリ一覧を確認する | 専用 companion ディレクトリが見える | 作成漏れ / 命名誤り | local entry files |
| mRL-0-2 | MRL-0 | ダッシュボード関連作業をこの配下だけで行うルールを確認できる | ルート docs とローカル AGENTS を確認する | 境界ルールが明文化される | 規約未反映 / 二重定義 | local docs boundary |
| mRL-1-1 | MRL-1 | ローカル `docs/` を入口に仕様整理を始められる | `iDevelop/docs/index.md` を確認する | source-of-truth 入口が存在する | docs 未整備 | local artifact docs |
| mRL-1-2 | MRL-1 | ローカル `develop/` を入口に実開発計画を始められる | `iDevelop/develop/index.md` を確認する | 計画と履歴入口が存在する | develop 未整備 | local plan set |
