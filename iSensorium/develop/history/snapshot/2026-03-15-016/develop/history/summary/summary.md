...
## 2026-03-15-016 MRL-10 preview MVP 完了

- 目標動作: guarded replacement route の live preview MVP を実機確認まで閉じ、`MRL-10` を完了状態へ更新する
- 変更意図: recorder side preview tap と UI renderer 方式で成立した preview MVP を docs / develop / history に反映し、次の作業を新 dated plan set 定義待ちへ切り替える
- 背景理由: `2026-03-15-014` は active plan set として残っていたが、最終的に Xperia 5 III 実機で preview 更新、recording 成立、保存整合維持が確認できた
- 変更要約: `docs/observability/current_state.md`、`docs/artifact/system_blueprint.md`、`docs/artifact/story_release_map.md`、`docs/process/UX_check_work_flow.md`、`develop/index.md`、`develop/plans/2026-03-15-014/*.md` を完了状態へ更新した
- 影響文書: `docs/observability/current_state.md`, `docs/artifact/system_blueprint.md`, `docs/artifact/story_release_map.md`, `docs/process/UX_check_work_flow.md`, `docs/history/docs/summary/summary.md`, `develop/index.md`, `develop/history/summary/summary.md`, `develop/plans/2026-03-15-014/market_release_lines.md`, `develop/plans/2026-03-15-014/micro_release_lines.md`
- 期待効果: `MRL-10` は recorder-side preview MVP 完了 line として閉じられ、次の thread は新しい plan set か後続品質改善へ直接進める

## 2026-03-15-012 replacement route 保存アウトプット整合確認

- 目標動作: replacement route session の保存アウトプットが想定どおり揃っていることを docs / develop に反映し、残課題を UX blocker 2 件へ絞る
- 変更意図: 実機確認結果を source-of-truth と develop に反映し、次の実装対象を `switch crash` と `recording 中 preview 停止` に固定する
- 背景理由: required artifact と manifest file size は解消済みになった一方、live 文書はまだ user UX check ready の完了状態だけを示していた
- 変更要約: `docs/observability/current_state.md`、`docs/artifact/system_blueprint.md`、`docs/process/UX_check_work_flow.md`、`develop/index.md` を更新し、保存アウトプット完了と残 UX blocker を記録した
- 影響文書: `docs/observability/current_state.md`, `docs/artifact/system_blueprint.md`, `docs/process/UX_check_work_flow.md`, `docs/history/docs/summary/summary.md`, `develop/index.md`, `develop/history/summary/summary.md`
- 期待効果: 次の実装修正がデータ整合ではなく UX blocker 修正へ一直線につながる

## 2026-03-15-011 MRL-8 と MRL-9 完了

- 目標動作: guarded replacement runtime の stabilization を閉じ、利用者が replacement route を選んで UX check を開始できる状態まで `MRL-8` と `MRL-9` を完了させる
- 変更意図: shutdown / failure / refresh の終端処理を安定化し、app 内 route switch と UX check 文書を追加して user UX check ready 状態へ進める
- 背景理由: `MRL-7` 完了時点では runtime は接続済みだったが、利用者が replacement route を選ぶ導線と UX 観察手順がまだ閉じていなかった
- 変更要約: `RecordingCoordinator.kt` に終端安定化を追加し、`MainActivity.kt` と `activity_main.xml` に `Use guarded replacement route` switch を追加し、`CoreCameraTrialRuntimeTest.kt` と `python/test_session_parser.py` を更新し、`docs/observability/current_state.md`、`docs/process/UX_check_work_flow.md`、`develop/plans/2026-03-15-011/` を完了状態へ更新した
- 影響文書: `app/src/main/java/com/isensorium/app/MainActivity.kt`, `app/src/main/java/com/isensorium/app/RecordingCoordinator.kt`, `app/src/main/res/layout/activity_main.xml`, `app/src/main/res/values/strings.xml`, `app/src/test/java/com/isensorium/app/CoreCameraTrialRuntimeTest.kt`, `python/test_session_parser.py`, `docs/artifact/system_blueprint.md`, `docs/artifact/story_release_map.md`, `docs/observability/current_state.md`, `docs/process/UX_check_work_flow.md`, `docs/history/docs/summary/summary.md`, `develop/index.md`, `develop/history/summary/summary.md`, `develop/plans/2026-03-15-011/market_release_lines.md`, `develop/plans/2026-03-15-011/micro_release_lines.md`
- 期待効果: 次の step は利用者 UX 確認だけになり、観察結果に応じて採用 / guarded 継続 / rollback をすぐ判断できる

## 2026-03-15-010 MRL-7 guarded runtime wiring 完了

- 目標動作: `shared-camera-session-adapter` の裏で guarded replacement runtime を actual runtime として起動できる状態まで `MRL-7` を閉じる
- 変更意図: `CoreCameraTrialSessionAdapter` 実装後の完了状態を docs / develop に反映し、次段を `MRL-8 guarded upstream stabilization` へ進める
- 背景理由: `MRL-7` の実装・build・parser 回帰が完了しているのに、live 文書がまだ wiring 途中の状態を示していた
- 変更要約: `app/build.gradle.kts` で guarded runtime gate を有効化し、`CoreCameraTrialRuntime.kt` と `RecordingCoordinator.kt` に replacement runtime wiring を実装し、`docs/observability/current_state.md`、`docs/artifact/system_blueprint.md`、`docs/artifact/story_release_map.md`、`develop/index.md`、`develop/plans/2026-03-15-009/*.md` を完了状態へ更新した
- 影響文書: `app/build.gradle.kts`, `app/src/main/java/com/isensorium/app/CoreCameraTrialRuntime.kt`, `app/src/main/java/com/isensorium/app/GuardedUpstreamTrial.kt`, `app/src/main/java/com/isensorium/app/RecordingCoordinator.kt`, `docs/artifact/system_blueprint.md`, `docs/artifact/story_release_map.md`, `docs/observability/current_state.md`, `docs/history/docs/summary/summary.md`, `develop/index.md`, `develop/history/summary/summary.md`, `develop/plans/2026-03-15-009/market_release_lines.md`, `develop/plans/2026-03-15-009/micro_release_lines.md`
- 期待効果: `MRL-7` は rollback 可能な実 wiring 完了 line として閉じられ、次の作業は lifecycle / preview / refresh の stabilization に集中できる

## 2026-03-15-009 MRL-7 以降の実装計画追加

- 目標動作: `MRL-6` 完了後の次実装入口を `MRL-7 guarded replacement runtime wiring` として固定し、user UX check までの線形計画を develop 側へ追加する
- 変更意図: `2026-03-15-009` plan set を追加し、`MRL-7`、`MRL-8`、`MRL-9` の market / micro release line を明示する
- 背景理由: seam 準備 line は閉じていたが、replacement runtime 実装から user UX check までの active execution target が source-of-truth 上で未定義だった
- 変更要約: `docs/artifact/story_release_map.md`、`docs/observability/current_state.md`、`develop/index.md` を更新し、`develop/plans/2026-03-15-009/market_release_lines.md` と `micro_release_lines.md` を追加した
- 影響文書: `docs/artifact/story_release_map.md`, `docs/observability/current_state.md`, `develop/index.md`, `develop/plans/2026-03-15-009/market_release_lines.md`, `develop/plans/2026-03-15-009/micro_release_lines.md`, `docs/history/docs/summary/summary.md`, `develop/history/summary/summary.md`
- 期待効果: `iSensorium` 実装が `MRL-7 -> MRL-8 -> MRL-9` の順で迷わず継続でき、最終的に user UX check ready 状態まで運べる
...
- 変更対象: defect recovery 運用記録
- 実施内容: toggle crash を抑止する route switch guard を維持しつつ、shared camera capture session へ追加した preview surface を撤回して guarded replacement route を recording 成立優先へ戻した
- 実装ファイル: `app/src/main/java/com/isensorium/app/MainActivity.kt`, `app/src/main/java/com/isensorium/app/RecordingCoordinator.kt`, `app/src/main/res/layout/activity_main.xml`
- 判断根拠: Xperia 5 III 実機で追加 preview surface 案が `Failed to create capture session; configuration failed` を再現した
- 次: replacement route recording 中の live preview は別経路で再設計する
## 2026-03-15-014 MRL-10 preview MVP 計画

- 変更対象: active dated plan set
- 実施内容: `2026-03-15-014/market_release_lines.md` と `micro_release_lines.md` を追加し、`mRL-10-1 recorder side preview tap` から始める preview MVP 実装順を固定した
- 実装方針: `TrialCpuImageVideoRecorder` に preview tap を設け、shared camera capture session の surface 構成は変えない
- exit 基準: `640x480` 系 `5fps` 始動 preview、recording 成立、saved files / manifest 整合維持

## 2026-03-15-015 MRL-10 preview MVP 実装

- 変更対象: preview MVP 実装反映
- 実施内容: recorder 経路に preview tap を追加し、replacement route recording 中に UI preview を更新する実装を追加した
- 実装ファイル: `app/src/main/java/com/isensorium/app/CoreCameraTrialRuntime.kt`, `app/src/main/java/com/isensorium/app/RecordingCoordinator.kt`, `app/src/main/java/com/isensorium/app/MainActivity.kt`, `app/src/main/res/layout/activity_main.xml`
- 次: Xperia 5 III 実機で preview 更新を確認し、`mRL-10-3` を閉じる
