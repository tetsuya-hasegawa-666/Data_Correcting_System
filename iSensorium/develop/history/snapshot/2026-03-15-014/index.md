...
- active dated plan set: `2026-03-15-014 MRL-10 guarded replacement preview MVP`
- active market release target: `MRL-10 guarded replacement preview MVP`
- active micro release target: `mRL-10-1 recorder side preview tap`
- 解釈:
  ...
  - `2026-03-15-014` は、guarded replacement route の preview MVP を recorder side tap 方式で進める active plan set である。
- 強い guardrail:
...
実機確認では、replacement route session の required artifact と manifest file size 整合までは確認済みであり、`Use guarded replacement route` の短間隔 toggle crash は解消した。現在の残課題は replacement route recording 中の live preview 1 件である。

2026-03-15 の defect recovery では、shared camera capture session へ preview surface を直接追加する案も試したが、Xperia 5 III で `Failed to create capture session; configuration failed` を再現したため撤回した。`MRL-10` では shared camera capture session の target を増やさず、`TrialCpuImageVideoRecorder` に流れる保存用フレームから `640x480` 系 `5fps` 始動の preview MVP を作る。
