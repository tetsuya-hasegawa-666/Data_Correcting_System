...
## 2026-03-14-004 codex-dashboard-bootstrap-boundary

- target_behavior: sibling subproject 立ち上げ時の実開発境界管理
- intended_change: `Codex_Dashborad/` をメインプロジェクトと分離し、初期 plan set とローカル運用入口を持つ構成へ切り替える
- background_reason: Codex 向けダッシュボード開発を既存の Xperia 系計画と混在させると、計画参照と履歴追跡が破綻する
- change_summary: ルート側に subproject 分離ルールと bootstrap 計画を追加し、`Codex_Dashborad/` 配下へ独立した `docs/` `develop/` `AGENTS.md` を初期配置した
