...
class RecordingCoordinator(
            ...
            .also { useCase ->
                useCase.setAnalyzer(cameraExecutor) { image ->
                    currentSession?.let { session ->
                        ...
                        arCoreLogger.onFrame(frameTimestamp, session)
                    }
                    image.close()
                }
            }
    ...
    private inner class ArCoreLogger {
        private var session: RecordingSession? = null
        ...
        fun onFrame(frameTimestamp: FrameTimestamp, recordingSession: RecordingSession) {
            val activeSession = arSession ?: return
            if (frameTimestamp.elapsedRealtimeNanos - lastLoggedFrameAtNs < 200_000_000L) {
                return
            }
            try {
                ...
                sessionManager.appendArCorePose(
                    recordingSession,
                    ArCorePoseSample(
                        elapsedRealtimeNanos = frameTimestamp.elapsedRealtimeNanos,
                        wallTimeMillis = frameTimestamp.wallTimeMillis,
                ...
                lastLoggedFrameAtNs = frameTimestamp.elapsedRealtimeNanos
            } catch (error: Exception) {
                if (lastLoggedFrameAtNs == 0L) {
                    sessionManager.appendCollectorStatus(
                        recordingSession,
                        "arcore",
                        "update_failed_${error.javaClass.simpleName}",
                    )
                    lastLoggedFrameAtNs = frameTimestamp.elapsedRealtimeNanos
                }
            }
