...
2026-03-15 の defect recovery では、shared camera capture session へ preview surface を直接追加する案も試したが、Xperia 5 III で `Failed to create capture session; configuration failed` を再現したため撤回した。`MRL-10` では shared camera capture session の target を増やさず、`TrialCpuImageVideoRecorder` に流れる保存用フレームから `640x480` 系 `5fps` 始動の preview MVP を作る。実装は完了しており、次は Xperia 5 III 実機で preview 更新を確認する。
