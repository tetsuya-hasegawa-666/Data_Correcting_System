...
class RecordingCoordinator(
    ...
    private var advancedSensorsEnabled: Boolean = false
    ...
    fun setAdvancedSensorsEnabled(enabled: Boolean) {
    ...
    fun startSession() {
        ...
        session.extendedSensorsEnabled = advancedSensorsEnabled
        ...
        statusListener(
            SessionUiState(
                ...
                if (advancedSensorsEnabled) {
                    "Session created. Preparing video + IMU + GNSS + BLE + ARCore."
                } else {
                    "Session created. Preparing stable video + IMU recording."
                },
            ),
        )
        ...
        if (advancedSensorsEnabled) {
            ...
            sessionManager.appendCollectorStatus(session, "gnss", "disabled_in_stable_mode")
            sessionManager.appendCollectorStatus(session, "ble", "disabled_in_stable_mode")
            sessionManager.appendCollectorStatus(session, "arcore", "disabled_in_stable_mode")
        }
                ...
                is VideoRecordEvent.Start -> {
                    ...
                    statusListener(
                        SessionUiState(
                            ...
                            toastMessage = if (session.extendedSensorsEnabled) {
                                "Extended collectors active"
                            } else {
                                "Stable recording active"
                            },
                        ),
                    )
                }
                    ...
                    statusListener(
                        SessionUiState(
                            ...
                            statusText = if (event.hasError()) {
                                ...
                                if (session.extendedSensorsEnabled) {
                                    "Session finalized with extended sensors. Review directory."
                                } else {
                                    "Session finalized with stable mode. Review directory."
                                }
                            },
                            toastMessage = if (event.hasError()) {
                                null
                            } else if (session.extendedSensorsEnabled) {
                                "Video + IMU + GNSS/BLE/AR session saved"
                            } else {
                                "Video + IMU session saved"
                            },
                        ),
                    )
                }
...
data class RecordingSession(
    ...
    var extendedSensorsEnabled: Boolean = false,
) {
...
class SessionManager(private val context: Context) {
    ...
    fun writeInitialManifest(session: RecordingSession) {
        ...
        session.manifestFile.writeText(
            JSONObject()
                ...
                .put("extendedSensorsEnabled", session.extendedSensorsEnabled)
                .put(
                    "streams",
                    JSONArray()
    ...
    fun finalizeManifest(session: RecordingSession, status: String) {
        ...
        manifest.put("extendedSensorsEnabled", session.extendedSensorsEnabled)
        manifest.put(
            "files",
            JSONArray().apply {
    ...
    fun findLatestSession(): RecordingSession? {
        ...
        return RecordingSession(
            ...
            extendedSensorsEnabled = manifest?.optBoolean("extendedSensorsEnabled") ?: false,
        )
    }
