...
## 2026-03-15 Active Plan Set

- active dated plan set: `none fixed after 2026-03-15-004 completion`
- active market release target: `MRL-6 guarded upstream trial completed`
- active micro release target: `none fixed; next dated plan set must declare runtime wiring work`
- interpretation:
  - `2026-03-14-006` remains the historical validated baseline.
  - `2026-03-15-004` is now the completed preparation line for guarded upstream trial inside `iSensorium`.
  - the next execution entry point must be declared in a new dated plan set after `MRL-6` completion.
- hard guardrails:
  - keep rollback anchor `rollback-isensorium-pre-upstream-trial-2026-03-15-001` intact.
  - keep the frozen `CameraX + ARCore` route restorable behind the seam.
  - do not treat `coreCamera` package readiness as `iSensorium` runtime wiring completion.
